...
struct st_ma_tls_received_sessions
{
  struct
  {
    SSL_SESSION *session;
    time_t not_after;
  } session[MA_TLS_MAX_RECEIVED_SESSIONS];
...
static MA_TLS_RECEIVED_SESSIONS *ma_tls_received_sessions_new(MYSQL *mysql)
{
  ...
  if (ma_session_cache_key(mysql, rs->key))
  {
    free(rs);
    return NULL;
  }
...
static void ma_tls_received_sessions_free(MARIADB_TLS *ctls)
{
  ...
  for (i= 0; i < rs->session_count; i++)
    ma_tls_session_free(rs->session[i].session);
...
SSL_SESSION *ma_tls_session_cache_get(MARIADB_TLS *ctls)
{
  ...
  return ma_tls_session_take(rs->key);
}
                            ...
                            time_t not_after)
{
  ...
  rs->session[rs->session_count].session= session;
  rs->session[rs->session_count].not_after= not_after;
  rs->session_count++;
...
void ma_pvio_cache_tls_session(MYSQL *mysql)
{
  ...
  rs->authenticated= 1;
  for (i= 0; i < rs->session_count; i++)
    ma_tls_session_add(rs->key, rs->session[i].session,
                       rs->session[i].not_after);
  rs->session_count= 0;
}
...
my_bool ma_pvio_tls_connect(MARIADB_TLS *ctls)
{
  ...
  {
    ma_tls_session_clear(ctls->received_sessions->key);
    ma_tls_close(ctls);
    ma_tls_received_sessions_free(ctls);
  }
  return rc;
