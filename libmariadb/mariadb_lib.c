                              ...
                              uint fields)
{
  ...
  while (*(cp=net->read_pos) != 254 || pkt_len >= 8)
  {
    ...
    *prev_ptr=cur;
    ...
    {
      /* Abort if not enough packet bytes remain to safely read field length indicator */
      if (cp >= end_cp)
      {
        free_rows(result);
      ...
      {
        cur->data[field] = to;
        /* Add explicit cp > end_cp check to prevent unsigned pointer underflow */
  ...
  *prev_ptr=0;                    /* last pointer is null */
  ...
  /* validate EOF status packet length before parsing */
  if (pkt_len > 1)
  {
