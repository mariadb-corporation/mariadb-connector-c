...
#ifdef HAVE_NONBLOCK
  /* Initialize async_methods if async context exists (MYSQL_OPT_NONBLOCK was set before connection) */
  if (pvio->mysql->options.extension && pvio->mysql->options.extension->async_context)
    ma_pvio_tls_init_async(pvio->ctls);
#endif
  return 0;
}
/* }}} */
