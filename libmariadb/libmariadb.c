...
mysql_debug(const char *debug __attribute__((unused)))
{
  ...
  else if ((env = getenv("MYSQL_DEBUG")))
  {
    ...
    {
      char buff[80];
      strcpy(strmov(buff,"libmariadb: "),env);
  ...
  if (argc != 1)				/* If some default option */
  {
    ...
    while (*++option)
    {
      ...
      if (option[0][0] == '-' && option[0][1] == '-')
      {
        ...
        for (end= *option ; (end= strchr(end,'_')) ; )
          *end= '-';
        switch (ma_find_type(*option+2,&option_types,2)) {
      	case OPT_port:
      	  ...
      	  break;
      	case OPT_socket:
          OPT_SET_VALUE_STR(options, unix_socket, opt_arg);
      	  break;
      	case OPT_compress:
      	  options->compress=1;
      	  break;
      	case OPT_password:
          OPT_SET_VALUE_STR(options, password, opt_arg);
      	  break;
      	case OPT_pipe:
        ...
        case OPT_timeout:
      	  ...
      	  break;
      	case OPT_user:
          OPT_SET_VALUE_STR(options, user, opt_arg);
      	  break;
      	case OPT_init_command:
      	  ...
      	  break;
      	case OPT_host:
          OPT_SET_VALUE_STR(options, host, opt_arg);
      	  break;
      	case OPT_database:
          OPT_SET_VALUE_STR(options, db, opt_arg);
      	  break;
      	case OPT_debug:
...
#ifdef HAVE_SSL
      	case OPT_ssl_key:
          OPT_SET_VALUE_STR(options, ssl_key, opt_arg);
        break;
      	case OPT_ssl_cert:
          OPT_SET_VALUE_STR(options, ssl_cert, opt_arg);
          break;
      	case OPT_ssl_ca:
          OPT_SET_VALUE_STR(options, ssl_ca, opt_arg);
          break;
      	case OPT_ssl_capath:
          OPT_SET_VALUE_STR(options, ssl_capath, opt_arg);
          break;
        case OPT_ssl_cipher:
          OPT_SET_VALUE_STR(options, ssl_cipher, opt_arg);
          break;
        case OPT_ssl_fp:
        ...
        case OPT_ssl_pw:
          ...
          OPT_SET_VALUE_STR(options, charset_dir, opt_arg);
      	  break;
      	case OPT_charset_name:
          OPT_SET_VALUE_STR(options, charset_name, opt_arg);
      	  break;
        ...
        case OPT_shared_memory_base_name:
          OPT_SET_VALUE_STR(options, shared_memory_base_name, opt_arg);
          break;
        ...
        case OPT_default_auth:
          ...
          OPT_SET_VALUE_STR(options, bind_address, opt_arg);
      	  break;
        default:
...
mysql_shutdown(MYSQL *mysql, enum mysql_enum_shutdown_level shutdown_level)
{
  ...
  DBUG_RETURN(ma_simple_command(mysql, COM_SHUTDOWN, (char *)s_level, 1, 0, 0));
}
...
mysql_refresh(MYSQL *mysql,uint options)
{
  ...
  DBUG_RETURN(ma_simple_command(mysql, COM_REFRESH,(char*) bits,1,0,0));
}
...
mysql_kill(MYSQL *mysql,ulong pid)
{
  ...
  DBUG_RETURN(ma_simple_command(mysql, COM_PROCESS_KILL,buff,4,0,0));
}
...
mysql_dump_debug_info(MYSQL *mysql)
{
  ...
  DBUG_RETURN(ma_simple_command(mysql, COM_DEBUG,0,0,0,0));
}
                                    ...
                                    enum enum_mysql_set_option option)
{
  ...
  DBUG_RETURN(ma_simple_command(mysql, COM_SET_OPTION, buffer, sizeof(buffer), 0, 0));
}
